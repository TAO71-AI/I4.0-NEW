...
def ReadConfiguration(ConfigurationFile: str | None = None, Create: bool = True, SetDefault: bool = False) -> dict[str, Any]:
    ...
    if (ConfigurationFile != DEFAULT_SERVER_CONFIGURATION_FILE):
        logging.info(f"[config] Reading default configuration file `{ConfigurationFile}`.")
    ...
    if (not os.path.exists(ConfigurationFile)):
        if (Create):
            ...
            logging.info(f"[config] Writting configuration file `{defaultConfigFile}` => `{ConfigurationFile}`.")
    ...
    # Read the file
    logging.info(f"[config] Reading configuration file `{ConfigurationFile}`.")
    conf = {}
