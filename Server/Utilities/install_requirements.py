...
def GetPIPCommand(InstallIfNotFound: bool = True) -> str | None:
    ...
    for command in PIP_COMMANDS:
        logging.info(f"[requirements_installation] Testing PIP command `{command}`...")
        process = subprocess.run(command.split(" ") + ["--help"], text = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
        
        if (process.returncode == 0):
            logging.info(f"[requirements_installation] Got PIP command `{command}`!")
            return command
    
    logging.error("[requirements_installation] Could not find PIP command automatically.")
...
def InstallPIP() -> None:
    ...
    for command in INSTALL_PIP_COMMANDS:
        logging.info(f"[requirements_installation] Trying to install PIP with the command `{command}`...")
        process = subprocess.run(command.split(" "))

        if (process.returncode == 0):
            logging.info(f"[requirements_installation] PIP installed with the command `{command}`!")
            return
    
    logging.error("[requirements_installation] Could not install PIP automatically.")
...
def InstallPackage(
    ...
    if (PIPCommand is None):
        logging.info("[requirements_installation] PIP command is None. Trying to get automatically.")
        PIPCommand = GetPIPCommand(True)

        if (PIPCommand is None):
            logging.error("[requirements_installation] Could not get PIP command automatically.")
    ...
    if ("--verbose" in PIPOptions):
        logging.info(f"[requirements_installation] Executing command `{' '.join(f'{k}={v}' for k, v in env.items())} {' '.join(cmd)}`")

    for name, value in EnvVars.items():
        env[name] = value

    logging.info(f"[requirements_installation] Executing installation command `{cmd}`...")
    process = subprocess.run(cmd, env = env, stderr = subprocess.PIPE)

    if (process.returncode != 0):
        logging.error(f"[requirements_installation] Error executing installation command:\n```plaintext\n{process.stderr.decode('utf-8')}\n```")
        raise exceptions.InstallationError(f"Error executing installation command:\n```plaintext\n{process.stderr.decode('utf-8')}\n```")
    
    logging.info("[requirements_installation] Package installed!")